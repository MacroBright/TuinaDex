"""MotionSafetySupervisor 运动安全监管者 (唯一底座动作安全出口).

核心职责：
- 回答“允许不允许执行”；
- 实施 22 轴软件硬限位/软限位 Clamping；
- 实施单步最大关节跳变限幅 (max_dq) 与角速度/角加速度硬限幅；
- 提供 CLAMP / REJECT / ESTOP 三级动作安全裁决；
- 记录动作来源 (source: ControlSource) 与遥测日志；
- 确保任何下发到 RealArmAdapter 与 LeapHandAdapter 的目标绝对安全。
"""

from __future__ import annotations

from dataclasses import dataclass, field
from enum import Enum
import time
from typing import Optional, Tuple
import numpy as np

from .control_arbiter import ControlSource


class SafetyDecision(Enum):
    """三级动作安全裁决枚举."""
    ALLOW = "ALLOW"      # 完全合法，原样放行
    CLAMP = "CLAMP"      # 轻微超限/越界，经安全夹紧限幅后允许执行
    REJECT = "REJECT"    # 严重超限/跳变过大/无有效位姿基准，拒绝执行并保持当前安全位姿
    ESTOP = "ESTOP"      # 看门狗超时/硬件故障/急停信号，立即断电停机


@dataclass
class SupervisorLimits:
    """机械臂与灵巧手安全限位配置 (所有角度/速度统一采用国际标准单位 rad, rad/s)."""

    # 6 轴机械臂关节限位 [min_rad, max_rad]
    arm_limits_rad: np.ndarray = field(
        default_factory=lambda: np.array([
            [-0.017453, 6.283185],   # J1: [-1°, 360°]
            [-0.017453, 2.617994],   # J2: [-1°, 150°]
            [-0.017453, 2.094395],   # J3: [-1°, 120°]
            [-1.570796, 1.570796],   # J4: [-90°, +90°]
            [-0.017453, 3.141593],   # J5: [-1°, 180°]
            [-0.017453, 6.283185],   # J6: [-1°, 360°]
        ], dtype=np.float32)
    )

    # 16 轴灵巧手各关节物理弯曲弧度限位 [min_rad, max_rad]
    hand_limits_rad: np.ndarray = field(
        default_factory=lambda: np.tile(np.array([-0.20, 3.14159], dtype=np.float32), (16, 1))
    )

    # 单步最大允许关节角度跳变 (默认 30° = 0.5236 rad)
    max_dq_rad: float = 0.5235987755982988

    # 严重超限直接 REJECT 阈值 (如单步跳变超过 60° = 1.047 rad)
    reject_dq_rad: float = 1.0471975511965976

    # 关节最大角速度 (默认 540°/s = 9.4247 rad/s)
    max_joint_vel_rad_s: float = 9.42477796076938

    # 限位安全裕度缓冲 (默认 2° = 0.0349 rad)
    margin_rad: float = 0.03490658503988659


@dataclass
class SupervisorResult:
    """监管检查结果."""

    decision: SafetyDecision              # 三级动作安全裁决
    is_safe: bool                         # 是否完全安全 (ALLOW 或 CLAMP)
    clamped_arm_q: np.ndarray             # 经限幅后的 6 轴机械臂目标弧度
    clamped_hand_q: np.ndarray            # 经限幅后的 16 轴灵巧手目标弧度
    arm_clamped: bool                     # 机械臂是否被截断
    hand_clamped: bool                    # 灵巧手是否被截断
    source: ControlSource = ControlSource.NONE  # 动作来源
    reason: str = ""                      # 截断或警告原因说明


class MotionSafetySupervisor:
    """机器人 22 轴运动安全主管."""

    def __init__(self, limits: Optional[SupervisorLimits] = None):
        self.limits = limits or SupervisorLimits()
        self._last_healthy_time = time.monotonic()
        self._watchdog_timeout_s = 0.500  # 看门狗超时熔断阈值

    def heartbeat(self, now: Optional[float] = None) -> None:
        """刷新看门狗心跳时间."""
        self._last_healthy_time = now if now is not None else time.monotonic()

    def is_watchdog_tripped(self, now: Optional[float] = None) -> bool:
        """检查看门狗是否发生超时断连."""
        t = now if now is not None else time.monotonic()
        return (t - self._last_healthy_time) > self._watchdog_timeout_s

    def validate_and_clamp_arm(
        self,
        target_q: np.ndarray,
        current_q: Optional[np.ndarray] = None,
        dt: float = 0.050,
    ) -> Tuple[np.ndarray, bool, bool, str]:
        """对 6 轴机械臂目标弧度进行限幅与跳变检查.

        Returns:
            (target, was_clamped, is_rejected, reason)
        """
        target = np.asarray(target_q, dtype=np.float32).copy()
        assert target.shape == (6,), f"Arm target_q shape must be (6,), got {target.shape}"

        was_clamped = False
        is_rejected = False
        reasons = []

        # 1. 软件软限位裁剪 (含 margin 缓冲)
        for i in range(6):
            min_q = self.limits.arm_limits_rad[i, 0] + self.limits.margin_rad
            max_q = self.limits.arm_limits_rad[i, 1] - self.limits.margin_rad
            if target[i] < min_q:
                target[i] = min_q
                was_clamped = True
                reasons.append(f"J{i+1} < {min_q:.3f}rad")
            elif target[i] > max_q:
                target[i] = max_q
                was_clamped = True
                reasons.append(f"J{i+1} > {max_q:.3f}rad")

        # 2. 差分速度与瞬时跳变限制
        if current_q is not None:
            curr = np.asarray(current_q, dtype=np.float32)
            delta_q = target - curr

            # 严重大跳变检查 -> 直接 REJECT
            for i in range(6):
                if abs(delta_q[i]) > self.limits.reject_dq_rad:
                    is_rejected = True
                    reasons.append(f"J{i+1} extreme jump {abs(delta_q[i]):.3f} > reject_dq {self.limits.reject_dq_rad:.3f}")
                    return curr.copy(), was_clamped, is_rejected, "; ".join(reasons)

            # 正常单步最大跳变限幅 (max_dq)
            for i in range(6):
                if abs(delta_q[i]) > self.limits.max_dq_rad:
                    target[i] = curr[i] + np.sign(delta_q[i]) * self.limits.max_dq_rad
                    was_clamped = True
                    reasons.append(f"J{i+1} jump {abs(delta_q[i]):.3f} > max_dq {self.limits.max_dq_rad:.3f}")

            # 瞬时角速度截断 (max_joint_vel)
            if dt > 1e-4:
                vel = (target - curr) / dt
                for i in range(6):
                    if abs(vel[i]) > self.limits.max_joint_vel_rad_s:
                        target[i] = curr[i] + np.sign(vel[i]) * self.limits.max_joint_vel_rad_s * dt
                        was_clamped = True
                        reasons.append(f"J{i+1} vel {abs(vel[i]):.3f} > max_vel")

        reason_str = "; ".join(reasons) if reasons else "OK"
        return target, was_clamped, is_rejected, reason_str

    def validate_and_clamp_hand(self, target_q: np.ndarray) -> Tuple[np.ndarray, bool, str]:
        """对 16 轴灵巧手物理弯曲弧度进行限幅."""
        target = np.asarray(target_q, dtype=np.float32).copy()
        assert target.shape == (16,), f"Hand target_q shape must be (16,), got {target.shape}"

        was_clamped = False
        reasons = []

        for i in range(16):
            min_q = self.limits.hand_limits_rad[i, 0]
            max_q = self.limits.hand_limits_rad[i, 1]
            if target[i] < min_q:
                target[i] = min_q
                was_clamped = True
                reasons.append(f"Hand_{i} < {min_q:.2f}")
            elif target[i] > max_q:
                target[i] = max_q
                was_clamped = True
                reasons.append(f"Hand_{i} > {max_q:.2f}")

        reason_str = "; ".join(reasons) if reasons else "OK"
        return target, was_clamped, reason_str

    def supervise_22d(
        self,
        action_22d: np.ndarray,
        current_state_22d: Optional[np.ndarray] = None,
        dt: float = 0.050,
        source: ControlSource = ControlSource.NONE,
    ) -> SupervisorResult:
        """对下发至机器人的 22 维动作执行严密物理与时序安全审查."""
        act_22d = np.asarray(action_22d, dtype=np.float32)
        assert act_22d.shape == (22,), f"Action shape must be (22,), got {act_22d.shape}"

        # 检查看门狗超时
        if self.is_watchdog_tripped():
            hold_arm = current_state_22d[:6].copy() if current_state_22d is not None else np.zeros(6, dtype=np.float32)
            hold_hand = current_state_22d[6:].copy() if current_state_22d is not None else np.zeros(16, dtype=np.float32)
            return SupervisorResult(
                decision=SafetyDecision.ESTOP,
                is_safe=False,
                clamped_arm_q=hold_arm,
                clamped_hand_q=hold_hand,
                arm_clamped=True,
                hand_clamped=True,
                source=source,
                reason="WATCHDOG_TRIPPED: No heartbeat received within timeout threshold",
            )

        curr_arm = current_state_22d[:6] if current_state_22d is not None else None
        target_arm = act_22d[:6]
        target_hand = act_22d[6:]

        clamped_arm, arm_clamped, arm_rejected, reason_arm = self.validate_and_clamp_arm(
            target_q=target_arm, current_q=curr_arm, dt=dt
        )
        clamped_hand, hand_clamped, reason_hand = self.validate_and_clamp_hand(target_q=target_hand)

        if arm_rejected:
            # 触发 REJECT 严重超限拒绝
            hold_arm = current_state_22d[:6].copy() if current_state_22d is not None else np.zeros(6, dtype=np.float32)
            hold_hand = current_state_22d[6:].copy() if current_state_22d is not None else np.zeros(16, dtype=np.float32)
            return SupervisorResult(
                decision=SafetyDecision.REJECT,
                is_safe=False,
                clamped_arm_q=hold_arm,
                clamped_hand_q=hold_hand,
                arm_clamped=True,
                hand_clamped=True,
                source=source,
                reason=f"REJECTED: {reason_arm}",
            )

        any_clamped = arm_clamped or hand_clamped
        decision = SafetyDecision.CLAMP if any_clamped else SafetyDecision.ALLOW
        combined_reason = f"Arm: {reason_arm} | Hand: {reason_hand}"

        return SupervisorResult(
            decision=decision,
            is_safe=True,
            clamped_arm_q=clamped_arm,
            clamped_hand_q=clamped_hand,
            arm_clamped=arm_clamped,
            hand_clamped=hand_clamped,
            source=source,
            reason=combined_reason,
        )
